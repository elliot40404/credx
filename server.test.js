...
app.post('/login', checkNotSignIn, async (req, res) => {
...
app.post('/signup', checkNotSignIn, async (req, res) => {
    let users = {
        cred: []
    }
    ...
    const mail = await req.body.email;
    const pass = await req.body.pass;
    const filename = mail.replaceAll('.', '_').replaceAll('@', '_') + '.json';
    const fn = __dirname + '/users.json'
...
app.post('/app', checkSignIn, async (req, res) => {
    let apps = {
        cred: []
    }
    const name = await req.body.name;
